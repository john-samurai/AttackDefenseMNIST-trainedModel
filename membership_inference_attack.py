...
class MembershipInferenceAttack:
    """
    Main class implementing the membership inference attack
    """
        ...
        for i, (shadow_model, train_data, test_data) in enumerate(self.shadow_models):
            ...
            with torch.no_grad():
                for data, _ in train_loader:
                    if member_count >= 200:  # Limit samples per model
                        break
            ...
            with torch.no_grad():
                for data, _ in test_loader:
                    if nonmember_count >= 200:  # Limit samples per model
                        break
    ...
    def evaluate_attack_effectiveness(self):
        """
        Comprehensive evaluation of the membership inference attack
        """
        print("\n" + "="*60)
...
def main():
    """
    Main function to demonstrate the membership inference attack
    """
    print("="*70)
    print("MEMBERSHIP INFERENCE ATTACK ON NEURAL NETWORK (MNIST)")
    print("="*70)
    print("Simulating bank's check processing model privacy attack...")
    
    # Initialize attack
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")
    
    attack = MembershipInferenceAttack(device=device)
    
    # Load or train target model (bank's check processing model)
    target_loaded = attack.load_target_model('mnist_cnn.pt')
    
    if not target_loaded:
        print("Failed to load target model. Exiting...")
        return
    ...
    # Step 1 & 2: Prepare shadow data and train shadow models
    shadow_datasets = attack.prepare_shadow_data(num_models=3, samples_per_model=1000)
    attack.train_shadow_models(shadow_datasets)
    
    # Step 3: Generate attack training data and train attack model
    attack_features, attack_labels = attack.generate_attack_training_data()
    attack.train_attack_model(attack_features, attack_labels)
    
    # Step 4: Evaluate attack effectiveness
    ...
    if accuracy > 0.7:
        print(f"HIGH PRIVACY RISK: Attack accuracy {accuracy:.2%} indicates significant privacy leakage!")
    elif accuracy > 0.6:
        print(f"MODERATE PRIVACY RISK: Attack accuracy {accuracy:.2%} shows measurable privacy leakage.")
    else:
        print(f"LOW PRIVACY RISK: Attack accuracy {accuracy:.2%} suggests limited privacy leakage.")
